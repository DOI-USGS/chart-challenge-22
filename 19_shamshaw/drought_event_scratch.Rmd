...
```{r}
startDate <- as.Date("2010-10-01")

target_threshold <- 5
event_subset <- events_crb_jd_1980 %>% 
  left_join(rdews_gages, by = "StaID", suffix = c("",".gages")) %>% 
  filter(HUC02 %in% c("Upper Colorado")) %>% 
  filter(start > startDate) %>%
  #filter(end <= "1990-10-01") %>% 
  filter(threshold == target_threshold) %>% 
  mutate(onset_day = as.integer(start - startDate)) %>% 
  mutate(end_day = as.integer(end - startDate)) %>% 
  arrange(onset_day, drought_id)

#t <- event_subset %>% group_by(onset_day) %>% summarize(n = n())
n <- 100

E <- as_tibble(matrix(NaN,nrow=n,ncol=max(event_subset$end_day)))
...
for (idx in 1:nrow(event_subset)){
  ...
  avail_rows <- E %>% select(all_of(temp_startd:(temp_startd + temp_dur-1)),priority,rnum) %>% 
    filter(is.na(if_any(starts_with("V")))) %>% 
    arrange(priority) %>% 
    group_by(priority) %>% 
    slice_sample(prop = 1) %>% 
    ungroup()
  temp_rnum <- avail_rows[[1,'rnum']]
  E[temp_rnum,(temp_startd:(temp_startd + temp_dur-1))] <- event_subset[[idx,'duration']]
}
...
hbreaks <- BAMMtools::getJenksBreaks(plot_dat$duration, k=10)
scaledBreaks <- scales::rescale(hbreaks, c(0,1))

p <- plot_dat %>% ggplot()+
  geom_tile(aes(x=dt, y=rnum, fill = duration), height=0.75)+
...
p
```
```{r, fig.cap="Monthly streamflow percentile (Julian day method) for gages along the Colorado River"}
# select streamflow percentile data for gages along the Colorado River
plot_dat <- all_streamflow %>% 
  left_join(rdews_gages, by = "StaID", suffix = c("",".gages")) %>% 
...
p <- streamgraph(plot_dat, "StaID", "perc_7day", "date", interactive=TRUE) %>%
  sg_axis_x(20, "date", "%b-%Y") %>%
  sg_fill_manual(sample(c_pal3))
p
```
...
plot_dat <- all_streamflow %>% 
  left_join(rdews_gages, by = "StaID", suffix = c("",".gages")) %>% 
  filter(StaID %in% c("09037500", "09038500")) %>% 
  select(dt, StaID, weibull_site) %>% 
  mutate(perc = 20 - weibull_site) %>% 
  mutate(perc = ifelse(perc <0, 0, perc)) %>% 
  filter((dt >= "2017-10-01" & dt <= "2019-09-30"))
