...
AK_map = ggplot(data = AK_sites_sf) + 
  geom_sf(data = outline, color = "lightgrey", fill = "midnightblue")+#fill = NA
  geom_sf(data = AK_shp, 
          color = "lightgrey",
          fill = "midnightblue") + #fill = NA, alpha = 0.7
  theme_void() + 
  theme(legend.position="top") +
  geom_sf(data = NA_rivers, col ='white', stroke = 0.1, lwd = 0.2) + 
  geom_sf(data = rivers10, col = 'white', stroke = 0.1, lwd = 0.2) +
  geom_sf(data = AK_SWORD_reaches, col = "dodgerblue1", stroke = 3, lwd = 2, alpha = 0.5) +
  geom_sf(color = "grey", alpha = 0.7) +
  labs(x = "Longitude", y = "Latitude", colour="gage end") +
  geom_sf(data = current_sites_sf, color = "goldenrod1", size = 2, alpha = 0.8) +
...
area_timeline = ggplot(combined_df, aes(x = year, y = upstream_area)) + 
  ...
  ylab("") + 
  annotate("text", x = 1990, y = 800000, label = "Data from USGS gages", col = "black") + 
  annotate("text", x = 2020, y = 100000000, label = "Data from SWOT", col = "black", hjust = 1)+
...
combined_order_df = rbind(order_df %>% mutate(type = 'gage'), SWOT_order_df %>% mutate(type = 'SWOT'))
combined_order_df = combined_order_df %>% filter(year > 1949)

order_long = combined_order_df %>% pivot_longer(cols = c(2:7), names_to = "order", values_to = "count")

order_long$order_number = gsub("order","",as.character(order_long$order))

order_timeline = ggplot(order_long, aes(x = year, y = count, color = order_number)) + 
  geom_line() + 
  theme_classic(base_size = 16) + 
  xlab("Year") + 
  ggtitle("Monitoring of large river reaches")+
  ylab("Number of River Reaches") + 
    annotate("text", x = 1990, y = 20, label = "Gaged rivers", col = "black") + 
  annotate("text", x = 2020, y = 750, label = "SWOT rivers", col = "black", hjust = 1)+
...
ggdraw() +
  ...
  draw_plot(AK_map+theme(legend.position = 'none',
            ...
            x = 0.02, y = -0.1, 
            height = 1.1) +
  draw_plot(order_timeline +#area_timeline +
