...
AK_map = ggplot(data = AK_sites_sf) + 
  ...
  geom_sf(data = current_sites_sf, col = "redorange", shape = 21) +
...
combined_df = rbind(upstream_df %>% mutate(type = 'Gages'), SWOT_df %>% mutate(type = 'SWOT'))
...
area_timeline = ggplot(combined_df, aes(x = year, y = upstream_area)) + 
  ...
  theme_bw() + 
  xlab("Year") + 
  ggtitle("Cumulative Upstream Area of \nObserved River Flow (1,000km2)") + 
  ylab("") + 
  annotate("text", x = 1990, y = 1000, label = "Total discharge captured by gages", col = "dimgray") + 
  annotate("text", x = 2000, y = 50000, label = "Increased monitoring with SWOT ->", col = "dimgray")+
